...
export const Route = createFileRoute("/chat/$conversationId")({
  ...
  errorComponent: () => (
    <div className="flex h-full items-center justify-center bg-[#0b101b] px-6 text-center text-sm text-slate-400">
...
function ConversationScreen() {
  ...
  // جلب رسائل المحادثة الحالية وحفظها في الكاش لسرعة الفتح
  const { data: storedMessages, isPending } = useQuery({
  ...
  useEffect(() => {
    if (storedMessages) {
      const formatted: Message[] = storedMessages.map((msg: StoredMessage) => ({
