...
function ChatPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const responseText = await askSalmanAI(newMessages);
      setMessages([...newMessages, { role: "assistant", content: responseText }]);
    } catch (error: any) {
      setMessages([
        ...newMessages,
        { role: "assistant", content: `خطأ: ${error.message || "عذراً، تعذر جلب الرد حالياً."}` },
      ]);
    } finally {
      setLoading(false);
    }
  ...
  return (
    <div className="flex flex-col h-screen max-w-3xl mx-auto p-4 rtl">
      ...
      <div className="flex-1 overflow-y-auto py-4 space-y-4">
        ...
        {loading && (
          <div className="p-4 rounded-xl max-w-[85%] text-sm bg-gray-100 dark:bg-gray-800 text-gray-500 ml-auto animate-pulse">
            جاري جلب الرد من Salman AI...
          </div>
        )}
      </div>
